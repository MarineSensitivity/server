...
| `/share/data/big/{version}/sdm.duckdb` | `rclone` sync | Species distribution model database (~5 GB); currently `v4` |
| `/share/data/derived/{version}/*.gpkg, *.csv, *.tif` | `rclone` sync | Derived spatial datasets; currently `v4` |
...
The BOEM server's root partition (`/`) is only **17 GB**. During the first `rclone` sync, the disk filled completely, causing sync failures and preventing new container pulls. The `sync-pull.sh` script syncs only the version specified by the `DATA_VERSION` variable (currently `v4`), excluding older dataset versions.
:::
