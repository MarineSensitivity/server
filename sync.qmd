...
%%| fig-cap: "Overall server architecture showing developer laptop, external AWS server, and internal BOEM server with Docker services and data flows."
%%| fig-width: 10
...
flowchart TB
  ...
  subgraph EXTERNAL["fa:fa-cloud External Server (AWS)"]
    ...
    subgraph EXT_DOCKER["fa:fa-docker Docker Services"]
      direction LR
      E_CADDY["fa:fa-server Caddy\n(reverse proxy)"]
      E_SHINY["fa:fa-chart-bar Shiny\n(apps)"]
      E_RSTUDIO["fa:fa-laptop-code RStudio\n(dev/debug)"]
      E_PLUMBER["fa:fa-plug Plumber\n(API)"]
    ...
    subgraph EXT_DATA["fa:fa-folder-open /share Data"]
      direction LR
      E_DUCKDB["fa:fa-database DuckDB\n(.duckdb)"]
      E_PMTILES["fa:fa-map PMTiles\n(.pmtiles)"]
      E_FILES["fa:fa-file GeoPackage\nCSV / TIF"]
      E_WWW["fa:fa-globe Static Web\n(docs, site)"]
    end

    subgraph EXT_GH["fa:fa-code-branch GitHub Actions"]
      E_GHCR["fa:fa-box GHCR\n(container registry)"]
    end
  end
  ...
  subgraph INTERNAL["fa:fa-shield-halved Internal Server (BOEM)"]
    ...
    subgraph INT_DOCKER["fa:fa-docker Docker Services (prod)"]
      direction LR
      I_CADDY["fa:fa-server Caddy\n(reverse proxy +\nfile server)"]
      I_SHINY["fa:fa-chart-bar Shiny\n(apps)"]
      I_WATCH["fa:fa-eye Watchtower\n(image updater)"]
    end

    subgraph INT_DATA["fa:fa-folder-open /share Data"]
      direction LR
      I_DUCKDB["fa:fa-database DuckDB\n(.duckdb)"]
      I_PMTILES["fa:fa-map PMTiles\n(.pmtiles)"]
      I_FILES["fa:fa-file GeoPackage\nCSV / TIF"]
      I_WWW["fa:fa-globe Static Web\n(docs, site)"]
    end

    subgraph INT_SYNC["fa:fa-sync Sync Services"]
      direction LR
      I_CRON["fa:fa-clock Cron"]
      I_RCLONE["fa:fa-exchange-alt rclone\n(sftp)"]
  ...
  %% developer -> external
  L_PUSH -->|"git push +\nCI/CD build"| E_GHCR
  L_PUSH -->|"git push"| EXT_DOCKER

  %% internal pulls from external
  ...
  style EXT_GH fill:#FFF8E1,stroke:#F9A825,stroke-width:1px
...
```{mermaid}
%%| label: fig-sync
%%| fig-cap: "Synchronization flow between internal BOEM server and external AWS server. All connections are initiated by the internal server (outbound only)."
%%| fig-width: 10
...
flowchart LR
  ...
  subgraph INTERNAL["fa:fa-shield-halved BOEM Internal Server"]
    ...
    PULL["fa:fa-download sync-pull.sh\n(rclone sftp)"]
    PUSH["fa:fa-upload sync-push.sh\n(rclone sftp)"]
    PING["fa:fa-heartbeat ping.sh\n(heartbeat)"]
    TOWER["fa:fa-eye Watchtower\n(Docker image pull)"]
  ...
  subgraph EXTERNAL["fa:fa-cloud External AWS Server"]
    ...
    subgraph PULL_TARGETS["fa:fa-folder Data + Content"]
      DATA["fa:fa-database Data Files\nduckdb / gpkg / csv / tif"]
      PMT["fa:fa-map PMTiles"]
      WWW["fa:fa-globe Website + Docs\n(static HTML)"]
      APPS["fa:fa-window-maximize Shiny Apps\n(R source)"]
    end

    subgraph PUSH_TARGETS["fa:fa-file-alt Logs"]
      LOGS["fa:fa-scroll Sync Logs\nShiny Logs\nDocker Logs"]
      HB["fa:fa-heartbeat heartbeat.json"]
    end

    subgraph IMAGES["fa:fa-docker Container Registry"]
      GHCR["fa:fa-box GHCR\nghcr.io/marinesensitivity"]
    end

    MON["fa:fa-bell monitor-heartbeat.sh\n(alerts if stale)"]
    HB --> MON
  end

  %% pull connections (internal initiates)
  PULL ==>|"pull data"| DATA
  PULL ==>|"pull pmtiles"| PMT
  PULL ==>|"pull website"| WWW
  PULL ==>|"pull apps"| APPS

  %% push connections (internal initiates)
  PUSH -.->|"push logs"| LOGS
  PING -.->|"push heartbeat"| HB
  ...
  subgraph LEGEND["Legend"]
    direction LR
    LP["fa:fa-arrow-right Solid = pull (data in)"]
  ...
  style IMAGES fill:#FFF8E1,stroke:#F9A825,stroke-width:1px
...
flowchart TB
  ...
  subgraph DOCKER["fa:fa-docker Docker (Production)"]
    CADDY["fa:fa-server Caddy\nport 80/443\n- reverse proxy\n- static file server\n- TLS termination"]
    SHINY["fa:fa-chart-bar Shiny\nport 3838\n- mapgl app\n- mapsp app"]
    WATCH["fa:fa-eye Watchtower\n- auto image updates"]
  end

  subgraph DATA["/share Volume"]
    direction LR
    DUCKDB["fa:fa-database DuckDB\nsdm.duckdb"]
    PMTILES["fa:fa-map PMTiles\n*.pmtiles"]
    STATIC["fa:fa-globe Static HTML\ndocs / website"]
    FILES["fa:fa-file Ancillary\ngpkg / csv / tif"]
  end
